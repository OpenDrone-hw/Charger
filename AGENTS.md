...
- Two KiCad projects. Charger node: root `hardware/Charger.kicad_sch`, main sheet `hardware/main.kicad_sch`, cell sheet `hardware/cell.kicad_sch` (6 instances), board `hardware/Charger.kicad_pcb` (layout in progress). Manager: root `hardware-manager/Manager.kicad_sch`, main sheet `hardware-manager/main.kicad_sch`, cell sheet `hardware-manager/cell.kicad_sch` (8 instances), no PCB.
...
- No fabrication exports exist. When they do, they land in `hardware/production/` (gitignored), generated with the KiCad Fabrication Toolkit.
