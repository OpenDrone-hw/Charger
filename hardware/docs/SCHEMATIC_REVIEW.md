...
### SLAVE (`/Users/stan/OpenDrone/Charger/hardware`, ESP32-C3, 2-6S / ≤3A charge-only)
...
### MANAGER (`/Users/stan/OpenDrone/Charger/hardware-manager`, ESP32-S3, 2-8S / ~10A, USB-PD 240W EPR bidirectional)
...
*Fix:* In KiCad GUI, run Tools ▸ Annotate Schematic (reset/re-link instances) so cell paths point at root `a9adf4ae…`, reconcile `.kicad_pro top_level_sheets` UUID, re-export netlist, confirm 54 unique cell refs and no warning. **Re-run this entire SLAVE audit afterward** — `slave.json`/`charger_slave.net`/`slave_FULL_pins.md` are unreliable for the cell hierarchy until then.
